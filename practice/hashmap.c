 ...
 *
 * ПРИЧИНЫ ВЫБОРА:
 *   1. Отличное распределение — минимальное количество коллизий
 *   2. Высокая производительность — работает за O(n), где n — длина строки
 *   3. Устойчивость к коллизиям — сложно подобрать ключи с одинаковым хешем
 *   4. Простота реализации
 *
 * КАК РАБОТАЕТ С ЧИСЛОВЫМИ КЛЮЧАМИ:
 *   1. Число преобразуется в строку через sprintf()
 *   2. Строка обрабатывается алгоритмом MurmurHash3:
 *      - Начальное значение: 0x12345678
 *      - Для каждого символа: hash ^= код_символа
 *      - Затем: hash *= 0x5bd1e995 (магическое число)
 *      - И: hash ^= hash >> 15 (смешивание битов)
 *   3. Индекс = hash % bucket_count
 *
 * ОБРАБОТКА КОЛЛИЗИЙ:
 *   Используется открытая адресация с линейным пробированием.
 *   При коллизии ищется следующая свободная ячейка.
 *   Удалённые ячейки помечаются флагом deleted.
 *   Это эффективно при нагрузке < 0.75.
 */
